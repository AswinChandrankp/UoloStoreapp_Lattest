          ...
          Screenshot(
            ...
            child: Container(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text(store.name!, style: PoppinsMedium.copyWith(fontSize: fontSize)),
                Text(store.address!, style: PoppinsRegular.copyWith(fontSize: fontSize)),
                Text(store.phone!, style: PoppinsRegular.copyWith(fontSize: fontSize)),
                Text(store.email!, style: PoppinsRegular.copyWith(fontSize: fontSize)),
                const SizedBox(height: 10),

                Row(children: [
                  Text('${'order_id'.tr}:', style: PoppinsRegular.copyWith(fontSize: fontSize)),
                  const SizedBox(width: 2),
                  Expanded(child: Text(order!.id.toString(), style: PoppinsMedium.copyWith(fontSize: fontSize))),
                  Text(DateConverterHelper.dateTimeStringToMonthAndTime(order!.createdAt!), style: PoppinsRegular.copyWith(fontSize: fontSize - 2), textAlign: TextAlign.end,),
                ]),
                order!.scheduled == 1 ? Text(
                  '${'scheduled_order_time'.tr} ${DateConverterHelper.dateTimeStringToDateTime(order!.scheduleAt!)}',
                  style: PoppinsRegular.copyWith(fontSize: fontSize),
                ) : const SizedBox(),
                const SizedBox(height: 5),

                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text(order!.orderType!.tr, style: PoppinsRegular.copyWith(fontSize: fontSize)),
                  Text(order!.paymentMethod!.tr, style: PoppinsRegular.copyWith(fontSize: fontSize)),
                ]),
                const Padding(
                ...
                Align(
                  ...
                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(order!.deliveryAddress?.contactPersonName ?? '', style: PoppinsRegular.copyWith(fontSize: fontSize)),
                    Text(order!.deliveryAddress?.address ?? '', style: PoppinsRegular.copyWith(fontSize: fontSize)),
                    Text(order!.deliveryAddress?.contactPersonNumber ?? '', style: PoppinsRegular.copyWith(fontSize: fontSize)),
                  ]),
                ),
                const SizedBox(height: 10),

                Row(children: [
                  Expanded(flex: 1, child: Text('sl'.tr.toUpperCase(), style: PoppinsMedium.copyWith(fontSize: fontSize))),
                  Expanded(flex: 6, child: Text('item_info'.tr, style: PoppinsMedium.copyWith(fontSize: fontSize))),
                  Expanded(flex: 1, child: Text(
                    'qty'.tr, style: PoppinsMedium.copyWith(fontSize: fontSize),
                    textAlign: TextAlign.center,
                  )),
                  Expanded(flex: 2, child: Text(
                    'price'.tr, style: PoppinsMedium.copyWith(fontSize: fontSize),
                ...
                Text('thank_you'.tr, style: PoppinsRegular.copyWith(fontSize: fontSize)),
                const Padding(
                  padding: EdgeInsets.symmetric(vertical: 8),
                  child: DottedDivider(height: 1, dashWidth: 4, dashHeight: 1),
                ),
